...
2. 「コピー元」にマテリアルを1つ設定、「+ コピー先を追加」でコピー先のマテリアルを複数登録
   - または「アバターから選択」にアバターのルートを設定すると、使用中のマテリアル一覧が表示され、「元」（1つだけ）「先」（複数可）のチェックボックスでコピー元・コピー先を選べます
3. コピー元のシェーダープロパティ一覧がチェックボックスで表示されるので、コピーしたい項目を選択（全選択/全解除ボタンあり）
4. 「コピー実行」を押すと、選択したプロパティのみが全コピー先マテリアルへ一括反映されます（Undo対応）
