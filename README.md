...
アバターのルートを指定するだけで、使用している全マテリアルのテクスチャプロパティを集め、圧縮品質を一括で高品質（High Quality）に変更するエディタ拡張です。解像度（Max Size）など他のインポート設定には触れません。
...
2. 「アバター」にアバターのルートを設定すると、使用中のテクスチャ一覧と現在の圧縮品質が表示されます
3. 「圧縮品質を高品質に変更」を押すと、対象テクスチャ全件の圧縮品質が高品質に変更され、再インポートされます
