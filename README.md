...
2. 「アバター」にアバターのルートを設定すると、使用中のマテリアル一覧が表示されます
3. 各マテリアルの「元」（コピー元、1つだけ）「先」（コピー先、複数可）にチェックを入れて対象を選択
...
## TextureCompressionBatchSetter

アバターのルートを指定するだけで、使用している全マテリアルのテクスチャプロパティを集め、テクスチャの圧縮設定（Compression / Crunch Compression / Compressor Quality）を一括変更するエディタ拡張です。

解像度（Max Size）は各テクスチャの元画像の解像度に応じて自動決定されます。元画像と同じ解像度がUnityの選択肢（32〜8192）にあればそれを、なければ長辺を超える最も近い選択肢が使われます。

インポート設定（.meta）への変更のため、他のツールと異なりCtrl+Zでは元に戻せません。事前にプロジェクトのバックアップを取ってから使用することを強く推奨します。

### 使い方

1. メニューから `Tools > VRChatTools` を開き、一覧から「Texture Compression Batch Setter」を選んで開く
2. 「アバター」にアバターのルートを設定すると、使用中のテクスチャ一覧と、変更後のMax Sizeが表示されます
3. 「圧縮設定」でCompression / Use Crunch Compression / Compressor Qualityを設定
4. 「設定を一括適用」を押すと、対象テクスチャ全件のインポート設定が変更され、再インポートされます

## 免責事項

本リポジトリのツール群は主にClaude Codeによって実装されており、作者はレビュワーとして関わっています。
